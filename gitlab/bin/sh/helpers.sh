...
push_site_to_s3() {
    ...
    s3cmd sync --encoding=utf-8 --delete-removed --acl-public --guess-mime-type --no-mime-magic --recursive \
          --add-header="Cache-Control:public, must-revalidate, proxy-revalidate, max-age=0" \
...
tag_successful_pipeline() {
    ...
    if [ ${current_sha} != ${last_tag_commit} ]; then
        ...
        curl --header "Authorization: token ${GITHUB_TOKEN}" \
            --data "${post_data}" \
            https://api.github.com/repos/DataDog/corp-hugo/git/refs
...
rollback_env() {
    ...
    pull_artifact_from_s3
    push_site_to_s3
    notify_slack "Problem encountered :sadpanda: - Production rollback to ${last_tag} has SUCCEEDED"
    pass_step  "${FUNCNAME}"
}
